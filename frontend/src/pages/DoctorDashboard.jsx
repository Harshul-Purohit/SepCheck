...
import { Stethoscope, AlertTriangle, Users, Download, Activity, FileText, User as UserIcon, ShieldCheck, Clock, CheckCircle, XCircle, Send } from 'lucide-react';
         ...
         <div className="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden">
            <div className="px-8 py-6 bg-brand-50/50 border-b border-brand-100 flex items-center justify-between">
               <h3 className="font-bold text-brand-900 flex items-center gap-3">
                 <Clock className="w-5 h-5 text-brand-600" /> Pending Consultations
               </h3>
               <span className="text-[10px] font-black text-brand-400 tracking-widest uppercase">Action Required</span>
            </div>
            <div className="p-4 space-y-4">
               {consultations.filter(c => c.status === 'pending').length === 0 ? (
                 <div className="py-12 text-center text-slate-400 font-medium">No pending consultation requests.</div>
               ) : (
