...
def generate_medical_report_pdf(profile, report):
    buffer = io.BytesIO()
    ...
    c.drawString(inch, height - 2.5*inch, f"Age/Gender: {profile.age} yrs / {profile.gender}")
    c.drawString(inch, height - 2.7*inch, f"Report ID: #{report.id + 1000}")
    c.drawString(width - 2.5*inch, height - 2.3*inch, f"Date: {report.created_at.strftime('%Y-%m-%d %H:%M')}")
    c.drawString(width - 2.5*inch, height - 2.5*inch, f"Blood Group: {profile.blood_group}")

    # Physical Vitals Table
    c.setFont("Helvetica-Bold", 11)
    c.drawString(inch, height - 3.1*inch, "Clinical Vitals at time of assessment")
    
    # Vitals boxes
    v_y = height - 3.7*inch
    box_w = (width - 2*inch) / 4
    
    vitals = [
        ("WEIGHT", f"{profile.weight} kg"),
        ("HEIGHT", f"{profile.height} cm"),
        ("BP", profile.blood_pressure),
        ("TEMP", f"{profile.temperature} °F")
    ]
    
    for i, (label, val) in enumerate(vitals):
        x = inch + (i * box_w)
        c.setStrokeColor(colors.lightgrey)
        c.rect(x, v_y, box_w - 0.1*inch, 0.4*inch, stroke=1, fill=0)
        c.setFont("Helvetica-Bold", 8)
        c.drawCentredString(x + (box_w-0.1*inch)/2, v_y + 0.25*inch, label)
        c.setFont("Helvetica", 9)
        c.drawCentredString(x + (box_w-0.1*inch)/2, v_y + 0.08*inch, val)
    ...
    c.drawCentredString(width / 2, height - 4.4*inch, f"{report.risk_level.upper()} RISK")
    
    c.setFillColor(colors.black)
    c.setFont("Helvetica", 11)
    c.drawCentredString(width / 2, height - 4.7*inch, f"AI Diagnostic Probability: {int(report.probability_score * 100)}%")

    # Symptoms Summary
    c.setFont("Helvetica-Bold", 12)
    c.drawString(inch, height - 5.3*inch, "Clinical Summary & AI Insights")
    c.line(inch, height - 5.35*inch, width - inch, height - 5.35*inch)
    
    c.setFont("Helvetica", 10)
    text_object = c.beginText(inch, height - 5.6*inch)
    ...
    # Abnormal Values
    a_y_start = height - 6.8 * inch
    c.setFont("Helvetica-Bold", 12)
    c.drawString(inch, a_y_start, "Abnormal Indicators Detected")
    c.setFont("Helvetica", 10)
    y = a_y_start - 0.3 * inch
    ...
    c.drawCentredString(width / 2, 0.5*inch, "Confidential AI Screening Report. Validated by SepCheck Cloud Node.")
    c.drawCentredString(width / 2, 0.35*inch, "This is an AI generated screening result and MUST be reviewed by a human medical professional.")
